...
<?php 
	...
	foreach($rejectedleaverequests as $rows) {
	  	?>
	<tr><td></td><td><?php echo $rows['first_name'];?></td><td><?php echo date("d-m-Y", strtotime($rows['startdate']));?></td><td><?php echo date("d-m-Y", strtotime($rows['enddate']));?></td><td><?php echo $rows['noofdays'];?></td><td><?php echo $rows['vacationtype'];?></td><td><?php if(isset($rows['rejectremarks'])) echo $rows['rejectremarks'];?></td><td><a href="<?php echo site_url('vacation/editleave/'.$rows['id'].'/'.$rows['employee_id']) ?>">&nbsp;&nbsp;Edit</a>&nbsp;&nbsp;<a href="<?php echo site_url('vacation/deleteleave/'.$rows['id'].'/'.$rows['employee_id']) ?>">Delete</a>&nbsp;</td></tr>
		<?php
	}	
