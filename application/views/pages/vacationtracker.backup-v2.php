...
<?php echo form_open('vacation/trackvacationbyuser', array('class' => 'form-add-job')) ?>
	 ...
	 $vacdetails = "";
	  foreach($vacdates as $rows => $values) {
		  ...
		  $vacdetails.= "<tr>";
			  ...
			  $vacdetails.= "<td>";
			  for($i=1;$i<=$thismonth_noofdays;$i++)
				{
				...
				$vacdetails.= "</td><td>";
				for($i=1;$i<=$nextmonth_noofdays;$i++)
				{
					$m2 = sprintf("%02d",date('m',$d));
					$day2 = sprintf("%02d",$i);
					$y2 = date('Y');
					$dateformated2 = $y2.'-'.$m2.'-'.$day2;
						if(isset($ccc) && $ccc > 0) {
					if(in_array($dateformated2, $datesperperson[$person])){  /* if the date is included in vacation dates*/
						
						$vacdetails.= "<div title='".$dateformated2."' class='datecolumns highlighted'></div>";
					}
					else {
						$vacdetails.= "<div title='".$dateformated2."' class='datecolumns'></div>";
					}
						}
						else {
						$vacdetails.= "<div title='".$dateformated2."' class='datecolumns'></div>";
				...
				$vacdetails.= "</td><td>";
				for($i=1;$i<=$nextmonth_noofdays2;$i++)
				{
					$m3 = sprintf("%02d",date('m',$d2));
					$day3 = sprintf("%02d",$i);
					$y3 = date('Y');
					$dateformated3 = $y3.'-'.$m3.'-'.$day3;
					if(isset($ccc) && $ccc > 0) {
					if (in_array($dateformated3, $datesperperson[$person])){  /* if the date is included in vacation dates*/
						
						$vacdetails.= "<div title='".$dateformated3."' class='datecolumns highlighted'></div>";
					}
					else {
						$vacdetails.= "<div title='".$dateformated3."' class='datecolumns'></div>";
					}
					}
					else {
						$vacdetails.= "<div title='".$dateformated3."' class='datecolumns'></div>";
	...
	<?php foreach($vacationdetailsbyid as $rows => $values) {
		    $vacdet[$rows]['employee']  = $values['employee'];
